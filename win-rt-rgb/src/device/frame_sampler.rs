...
impl FrameSampler for VerticalFrameSampler {
    fn sample(&mut self, frame: &desktop_capture::Frame) -> RenderBuffer {
        ...
        for i in 0..self.buffer.len() {
            ...
            let section_start = (i as f64 * section_height).ceil() as usize;
            let section_end = ((i+1) as f64 * section_height).ceil() as usize;
            ...
            self.buffer[i].red   = (section_sum.red / 255)   as f32 / pixels_in_section as f32;
            self.buffer[i].green = (section_sum.green / 255) as f32 / pixels_in_section as f32;
            self.buffer[i].blue  = (section_sum.blue / 255)  as f32 / pixels_in_section as f32;
        }
        return self.buffer.clone();
    }
}